...
![Version: 1.43.2](https://img.shields.io/badge/Version-1.43.2-informational?style=flat-square) ![AppVersion: 1.0.0](https://img.shields.io/badge/AppVersion-1.0.0-informational?style=flat-square)
...
| podDisruptionBudget.maxUnavailable | int|string | `nil` | The maximum number of pods that can be unavailable. Can be an absolute number or a percentage (e.g., "1" or "10%") |
| podDisruptionBudget.minAvailable | int|string | `nil` | The minimum number of pods that must be available. Can be an absolute number or a percentage (e.g., "30%") |
| podDisruptionBudget.unhealthyPodEvictionPolicy | string | `nil` | Defines how unhealthy pods are considered for eviction. Allowed values: [ "IfHealthyBudget", "AlwaysAllow" ] |
...
When `type` is `generic`, the chart renders `templates/ingress-generic.yaml`.
...
Pod Disruption Budget (PDB) is a Kubernetes resource that ensures high availability of applications by defining the minimum number of pods that must remain available during voluntary disruptions (e.g., cluster upgrades, node drains, maintenance operations).

### 6.1 Basic Configuration

To enable PodDisruptionBudget creation for a microservice, set `podDisruptionBudget.create: true` in the _values.yaml_ file:
...
This configuration will create a PodDisruptionBudget resource that ensures at least 1 pod remains available at all times.
...
podDisruptionBudget:
  create: true
  maxUnavailable: "10%"        # At most 10% of pods can be unavailable
```

**Note:** Either `minAvailable` or `maxUnavailable` should be defined, but not both.

#### 6.2.3 unhealthyPodEvictionPolicy

Defines how unhealthy pods are considered for eviction. Allowed values are:

- `IfHealthyBudget` (default): Unhealthy pods can be evicted only if the budget for healthy pods is not yet met
- `AlwaysAllow`: Unhealthy pods can always be evicted, irrespective of the budget
...
#### Example 1: Ensure at least 2 pods are always available

```yaml
# /microservices/critical-service/prod/values.yaml
...
This configuration ensures that at least 2 pods of the microservice remain available at all times, even during cluster maintenance or upgrades.

#### Example 2: Allow at most one pod to be unavailable (percentage-based)

```yaml
# /microservices/standard-service/qa/values.yaml
...
This configuration allows up to 25% of pods to be unavailable during a disruption event.

#### Example 3: Allow graceful eviction of unhealthy pods

```yaml
# /microservices/resilient-service/dev/values.yaml
...
This configuration ensures at least 1 pod remains available, while still allowing unhealthy pods to be evicted to improve cluster stability.

### 6.4 Use Cases

- **Production deployments**: Use PDB to ensure services remain available during cluster updates
- **Multi-replica services**: Protect against accidental disruptions when scaling down nodes
- **Mission-critical applications**: Define strict availability requirements
- **Graceful maintenance**: Allow safe cluster upgrades without service interruption

---

## 7. Deployment Types

### 7.1. TechStack NodeJS
...
#### 7.1.1. <ins>Default NodeJS Microservice Deployment</ins>
...
#### 7.1.2. <ins>Generic Consumer Deployment</ins>
...
#### 7.1.3. <ins>Process Microservice Deployment</ins>
...
#### 7.1.4. <ins>Read Model Writer Deployment</ins>
