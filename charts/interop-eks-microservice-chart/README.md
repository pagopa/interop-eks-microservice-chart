...
| autoscaling.keda.scaledObjectSpec | object | `nil` | KEDA ScaledObject configuration |
| autoscaling.keda.scaledObjectSpec.minReplicaCount | int | `nil` | Minimum number of replicas for the service |
| autoscaling.keda.scaledObjectSpec.maxReplicaCount | int | `nil` | Maximum number of replicas for the service |
| autoscaling.keda.scaledObjectSpec.pollingInterval | int | `nil` | Polling interval (in seconds) for checking metrics |
| autoscaling.keda.scaledObjectSpec.scaleDownStabilizationWindowSeconds | int | `nil` | Stabilization window (in seconds) for scaling down |
| autoscaling.keda.scaledObjectSpec.scaleUpStabilizationWindowSeconds | int | `nil` | Stabilization window (in seconds) for scaling up |
| autoscaling.keda.scaledObjectSpec.triggers | list | `nil` | List of triggers for configuring autoscaling |
